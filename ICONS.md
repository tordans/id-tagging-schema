...
Introducing a new prefixed family (e.g. a new public icon set with its own npm package or CDN) requires changes to **documentation and build output**, **iD**, and often **other editors**. That process is documented on a separate page so this file stays short for contributors:

**[Adding a new icon source](docs/Adding-a-new-icon-source.md)** — Taginfo URLs, rollout conventions, consumer coordination, and a checklist.
